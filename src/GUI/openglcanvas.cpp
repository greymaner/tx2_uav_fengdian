...
OpenGLCanvas::OpenGLCanvas(Widget *parent, ThreadExchangeData* data) : nanogui::GLCanvas(parent)
{
    ...
    Resource vertShader = LOAD_RESOURCE(basicShader_vs);
    Resource fragShader = LOAD_RESOURCE(basicShader_fs);
    m_shader.init("raymarching_shader",
                 std::string(vertShader.data(), vertShader.size()),
                 std::string(fragShader.data(), fragShader.size())
    );
