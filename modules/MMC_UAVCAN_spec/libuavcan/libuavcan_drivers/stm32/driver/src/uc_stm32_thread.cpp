...
bool BusEvent::waitExt(uavcan::MonotonicDuration duration)
{
    ...
    {
# if (CH_KERNEL_MAJOR == 2)
        ret = sem_ext_.waitTimeout(TIME_IMMEDIATE);
# else // ChibiOS 3+
        ret = sem_ext_.wait(TIME_IMMEDIATE);
# endif
    }
    else
    {
# if (CH_KERNEL_MAJOR == 2)
        ret = sem_ext_.waitTimeout((msec > MaxDelayMSec) ? MS2ST(MaxDelayMSec) : MS2ST(msec));
# else // ChibiOS 3+
        ret = sem_ext_.wait((msec > MaxDelayMSec) ? MS2ST(MaxDelayMSec) : MS2ST(msec));
# endif
    }
# if (CH_KERNEL_MAJOR == 2)
    return ret == RDY_OK;
